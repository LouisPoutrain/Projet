...
class DraftPredictor:
    ...
    def _predict_ml(self, target_col, feature_cols, result_filter, feature_values):
        ...
        # Champions déjà utilisés dans le draft
        used = set(feature_values)

        # On enlève de la distribution tous ceux déjà pick
        for i, c in enumerate(classes):
            if c in used:
                probas[i] = 0

        # Si tout est éliminé → fallback : prédiction brute
        if probas.sum() == 0:
