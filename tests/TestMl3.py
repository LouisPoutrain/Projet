...
class DraftPredictor:
    ...
    def _predict_ml(self, target_col, feature_cols, result_filter, feature_values):
            ...
            # Encode les labels du champion (OBLIGATOIRE pour XGBoost)
            label_encoder = LabelEncoder()
            y_encoded = label_encoder.fit_transform(y)

            # OneHotEncoder sur les features
            ...
            # Exclure les champions déjà pickés
            used = set(feature_values)
            mask = np.array([c not in used for c in classes])
            probas = probas * mask

            # Si toutes probabilités à zéro → fallback
            if probas.sum() == 0:
                remaining = [c for c in classes if c not in used]
                return remaining[0]

            # Retour du champion original
            best_encoded = probas.argmax()
            return classes[best_encoded]
